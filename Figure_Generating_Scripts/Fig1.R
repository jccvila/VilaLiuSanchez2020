...
plot_comp_diff <- function(doc_df,t){
  ...
  return(ggarrange(p1,labels=c('G')))
}
...
plot_comp_self <- function(doc_df,t){
  ...
  p2 <- ggboxplot(B,x='Treatment',y='Dissimilarity',col='Treatment',palette = "jco",
    ...
    theme(axis.title.x=element_blank(),
          legend.position = '' ,
          axis.line = element_line(size=1),
          axis.text = element_text(size=9))  +    scale_y_continuous(breaks=c(0,0.8),limits=c(0,0.9))+
  ...
  return(ggarrange(p1,labels='E',ncol=1))
}
...
if(null == 0){
  save_plot('../Final Figures/Fig1.png',grid.arrange(f2,fig_3,heights = c(2,1)),base_height=13.5/2.8 * 3/2,base_width=27/2.8)
  }
if(null == 1){
  save_plot('../Final Figures/Supp1.png',grid.arrange(f2,fig_3,heights = c(2,1)),base_height=13.5/2.8 * 3/2,base_width=27/2.8)
}
# 