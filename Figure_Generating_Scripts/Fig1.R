...
plot_olm_cs<- function(dat,med){
  ...
  #media
  mG = lm(Dissimilarity ~ Overlap, G[Overlap>med])
  mC =lm(Dissimilarity ~ Overlap, C[Overlap>med])
  mL =lm(Dissimilarity ~ Overlap, L[Overlap>med])
  ...
  sdataG= fread( '../Stat_Outputs/Curve_Fitting_Glucose.csv')
  sdataC= fread( '../Stat_Outputs/Curve_Fitting_Citrate.csv')
  sdataL= fread( '../Stat_Outputs/Curve_Fitting_Leucine.csv')
  ...
  if(pvalueG ==0){pvalueG = ' < 0.002'}else{
    pvalueG = paste('p = ',signif(pvalueG,2),sep='')
  }
  if(pvalueC ==0){pvalueC = ' < 0.002'}else{
    pvalueC = paste('p = ',signif(pvalueC,2),sep='')
  }
  if(pvalueL ==0){pvalueL = ' < 0.002'}else{
  ...
  p<- ggplot(dat[Overlap>med]) +
    geom_point(data=dat[Overlap>med],aes(x= Overlap,y = Dissimilarity),size=1,alpha=0.05) +
    theme_bw() + 
    geom_line(data=sdata,aes(x=xs,y=mean_lm),col='Blue',size=2) +
    geom_ribbon(data=sdata,aes(x=xs,ymin=LCI_lm,ymax=UCI_lm),alpha=0.5,fill='Blue') +
    scale_y_continuous(breaks=c(0,0.8),limits=c(0,0.9)) +# theme(axis.line = element_line(size=1)) +
...
pB <- plot_lowess(doc_df2,'../Stat_Outputs/Curve_Fitting_SameEnv.csv',median(doc_df2$Overlap))
pBInset <- plot_olm(doc_df2,'../Stat_Outputs/Curve_Fitting_SameEnv.csv',median(doc_df2$Overlap))+  
...
pD <- plot_olm(doc_df2[Same_Inoculum==TRUE],'../Stat_Outputs/Curve_Fitting_SameEnvSameInoc.csv',median(doc_df2$Overlap))
pE <- plot_olm(doc_df2[Same_Inoculum==FALSE],'../Stat_Outputs/Curve_Fitting_SameEnvDiffInoc.csv',median(doc_df2$Overlap)) 
  
pF <- plot_comp_self(doc_df2,0.98)
pG <- plot_overlap_dists(doc_df)
pH <- plot_comp_diff(doc_df,0.98,fn = '../Stat_Outputs/TTest_CSource.csv')

t1 = text_grob(expression('' == 'Inoculum'),face='bold',size=12)
t2 = text_grob(expression('' != 'Inoculum'),face='bold',size=12)
pleft = ggarrange(pB + annotation_custom(arrangeGrob(pBInset),xmin=0.0,xmax=0.3,ymin=0.6,ymax=0.83),labels='B')
pmiddle = ggarrange(pC,labels='C')
pright = ggarrange(grid.arrange(pD,top=t1),grid.arrange(pE,top=t2),ncol=1,labels=c('D','E'))
pbottom = ggarrange(pF,pG,pH,labels=c('F','G','H'),ncol=3)
ggsave(filename = '../Final_Figures/Fig1.png',grid.arrange(pleft,pmiddle,pright,pbottom,
