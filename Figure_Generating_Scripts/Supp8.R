...
plot_cit <- function(dat,med){
  ...
  dat = dat[Overlap>med,]
  pA <- ggplot(dat,aes(x=Overlap,y=Dissimilarity,col=Raoultella)) + geom_point(size=3) +
...
plot_univ <- function(dat,fn1,fn2){
  ...
  tl = text_grob('Raoultella Communities',face='bold',size=12)
  tr = text_grob('No Raoultella Communities',face='bold',size=12)
...
if(null == 0){
  ...
  doc_df = doc_df[Carbon_Source_1 == 'Citrate' & Carbon_Source_2 == 'Citrate']
  doc_df = doc_df[Dissimilarity!=0]
  # pbottom = plot_univ(doc_df,'../Stat_Outputs/Raoul_Dat.csv','../Stat_Outputs/No_Raoul_Dat.csv')
  # pbottom = ggarrange(pbottom[[1]],pbottom[[2]],ncol=2,labels=c('C','D'))
}else{
  doc_df = fread('../Data/Dissimilarity_Overlap_Null.csv')
  doc_df = doc_df[Carbon_Source_1 == 'Citrate' & Carbon_Source_2 == 'Citrate']
  doc_df = doc_df[Dissimilarity!=0]
  # pbottom = plot_univ(doc_df,'../Stat_Outputs/Raoul_Dat.csv','../Stat_Outputs/No_Raoul_Dat_Null.csv')
  # pbottom = ggarrange(pbottom[[1]],pbottom[[2]],ncol=2,labels=c('A','B'))
}

ptop = plot_inc(doc_df[Same_Inoculum==TRUE])
pmid = plot_cit(doc_df,median(doc_df$Overlap))


if(null==0){ggsave('../Final Figures/Supp8.png',grid.arrange(pmid),width=9,height=4.5)
  }else{ggsave('../Final Figures/Supp9.png',grid.arrange(pbottom),width=8,height=4)
  }