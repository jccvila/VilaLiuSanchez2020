...
doc_df = doc_df[Carbon_Source_1 == 'Leucine' & Carbon_Source_2 == 'Leucine']
# doc_df_b = doc_df_b[Carbon_Source_1 == 'Leucine' & Carbon_Source_2 == 'Leucine']
...
plot_cit <- function(dat,med){
  ...
  dat$Pseudomonas <- 'One'
  dat[Var1 %!in% cit_coms & Var2 %!in% cit_coms,]$Pseudomonas= 'Neither'
  dat[Var1 %in% cit_coms & Var2 %in% cit_coms,]$Pseudomonas= 'Both'
  dat$Pseudomonas = factor(dat$Pseudomonas,levels=c('Both','One','Neither'))
  med =  floor(median(dat$Overlap)*100)/100
  dat = dat[Overlap>med,]
  pA <- ggplot(dat,aes(x=Overlap,y=Dissimilarity,col=Pseudomonas)) + geom_point(size=3) +
    ...
    theme(axis.title= element_text(size=16),axis.line = element_line(size=2),
          axis.text = element_text(size=14),legend.text = element_text(size=12)) + labs(col='Pseudomonas In') +
    scale_color_manual(values=c('#0073C2FF','#8EE5EE','#EFC000FF'))

  mycomparisons = list( c('Both','One'),c('One','Neither'),c('Both','Neither'))

  pB <- ggboxplot(dat,x='Pseudomonas',y='Dissimilarity',col='Pseudomonas',palette = "jco",
...
plot_univ <- function(dat,fn1,fn2){
  ...
  tl = text_grob('Pseudomonas Communities',face='bold',size=12)
  tr = text_grob('No Pseudomonas Communities',face='bold',size=12)
...
if(null == 0){
  ...
  doc_df = doc_df[Carbon_Source_1 == 'Leucine' & Carbon_Source_2 == 'Leucine']
  doc_df = doc_df[Dissimilarity!=0]
  # pbottom = plot_univ(doc_df,'../Stat_Outputs/Pseud_Dat.csv','../Stat_Outputs/No_Pseud_Dat.csv')
  # pbottom = ggarrange(pbottom[[1]],pbottom[[2]],ncol=2,labels=c('C','D'))
}else{
  doc_df = fread('../Data/Dissimilarity_Overlap_Null.csv')
  doc_df = doc_df[Carbon_Source_1 == 'Leucine' & Carbon_Source_2 == 'Leucine']
  doc_df = doc_df[Dissimilarity!=0]
  # pbottom = plot_univ(doc_df,'../Stat_Outputs/Pseud_Dat.csv','../Stat_Outputs/No_Pseud_Dat_Null.csv')
...
if(null==0){ggsave('../Final Figures/Supp9.png',grid.arrange(pmid),width=9,height=4.5)}