...
plot_high_overlap <- function(dat,med){
  ...
  dat = dat[Overlap>med,]
  pA <- ggplot() + geom_point(dat,mapping = aes(x=Overlap,y=Dissimilarity,col=Pseudomonas)) +
...
doc_df = doc_df[Dissimilarity!=0 & Same_Environment == TRUE & Carbon_Source_1 == 'Leucine',]
...
pB <- plot_comp(doc_df[Same_Inoculum==TRUE],
                med = median(doc_df[Same_Inoculum==TRUE]$Overlap),
                fn = '../Stat_Outputs/TTest_CSource_Pseud.csv')
pAB = ggarrange(pA,pB,labels = 'AUTO')

ggsave('../Final_Figures/Supp9.png',pAB,width=9,height=4)